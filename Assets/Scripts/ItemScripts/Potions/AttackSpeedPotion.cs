...
{
    [SerializeField] 
    private float attackSpeedIncreasement = 10.0f;    //공격속도 증가량 (퍼센트)
    ...
    {
        float forDebug = playerStatus.AdditionalAttackSpeed; //디버깅용
        playerStatus.AdditionalAttackSpeed += attackSpeedIncreasement;
        Debug.Log($"공격속도 {attackSpeedIncreasement}% 증가함: {forDebug} -> {playerStatus.AdditionalAttackSpeed}");
    }
}
